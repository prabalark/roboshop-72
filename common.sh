...
dnf module disable nodejs -y 2>/tmp/roboshop.log
dnf module enable nodejs:18 -y 2>/tmp/roboshop.log
dnf install nodejs -y 2>/tmp/roboshop.log
func_exit $?

print_head "create /app"
useradd ${username} 2>/tmp/roboshop.log
func_exit $?
